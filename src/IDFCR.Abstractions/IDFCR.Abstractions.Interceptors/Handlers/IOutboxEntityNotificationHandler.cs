    ...
    where TKey: struct
{
    ...
    /// </summary>
    /// <param name="id">The identifier of the outbox entity that has changed.</param>
...
public interface IOutboxEntityNotificationHandler
{
    ...
    /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
    /// <returns>A task that represents the asynchronous operation.</returns>
    Task<object?> NotifyAsync(object id, object entity, CancellationToken cancellationToken);
}
