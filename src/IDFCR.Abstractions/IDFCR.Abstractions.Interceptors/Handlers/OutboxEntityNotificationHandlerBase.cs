    ...
    where TKey : struct
{
    ...
    public abstract Task NotifyAsync(TEntity entity, CancellationToken cancellationToken);
    ...
    /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
    /// <returns>A task representing the asynchronous operation.</returns>
    public Task NotifyAsync(object entity, CancellationToken cancellationToken)
    {
        if (entity is TEntity typedEntity)
        {
            return NotifyAsync(typedEntity, cancellationToken);
        }
