...
AS BEGIN
	...
	BEGIN TRY
		BEGIN TRANSACTION;
		SELECT TOP(1) @packageId = PackageId FROM dbo.Package WITH (UPDLOCK, HOLDLOCK) WHERE Name = @trimmedPackageName;

		IF @packageId IS NULL
		BEGIN
			SET @packageId = NEWID(); 
			INSERT INTO dbo.Package ([PackageId], [Name], [Alias], [Description]) VALUES (@packageId, @trimmedPackageName, @packageAlias, @packageDescription);
			SET @revisionId = 0;
		END
		ELSE BEGIN
		...
		INSERT INTO [dbo].[PackageVersion] ([PackageId], [VersionSuffix], [RevisionNumber]) VALUES (@packageId, @versionPrefix, @revisionId);
		COMMIT TRANSACTION;
